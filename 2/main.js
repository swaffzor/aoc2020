...
*/
var part1Result = input.filter(function (entry) {
    var occurences = entry.policy.split(entry.target).length - 1;
...
*/
var part2Result = input.filter(function (entry) {
    var pw = entry.policy.split('');
    // console.log(entry, " => ", pw)
    var count = 0;
    if (entry.min - 1 < pw.length && pw[entry.min - 1] === entry.target)
        count++;
    if (entry.max - 1 < pw.length && pw[entry.max - 1] === entry.target)
        count++;
    return count === 1;
});
